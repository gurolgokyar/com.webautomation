...
public class RevertDraggablePositionTest extends Hooks {
	...
	void testDragFirstBoxInBigBox() {
		...
		// There would be negligible difference between last position and initial position
		// after the box reverted
		boolean isNegligibleDifference = pages.getRevertDraggablePosition()
			.isNegligibleDifference(initialPosition, finalPosition);
	...
	void testDragSecondBoxInBigBox() {
		...
		assertTrue(initialPosition.getY() < finalPosition.getY(),
				"Attempted to move second box in big box, but it " + "did not moved");
	...
	void testDragSecondBoxAfterDraggedInBigBox() {
		...
		assertTrue(isNegligibleDifference, "Second box should not move out of big box when it is located " + "there");
